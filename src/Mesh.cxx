...
auto get_or_create_material_id(MaterialIdTable& table, const std::string& name) -> u32 {
    ...
    if (it != table.name_to_id.end()) return it->second;
...
auto build_loaded_texture_table(std::span<const TextureLoadPacket> textures,
                                       std::span<const TextureHandle> handles) -> LoadedTextureTable {
...
auto to_gpu_material(const MaterialData& m,
                            const LoadedTextureTable& loaded,
                            const DefaultTextureHandles& defs) -> GPUMaterialData {
    GPUMaterialData out{};
    ...
    while (std::getline(file, line)) {
        ...
        if (token == "newmtl") {
            ...
            if (token == "map_Kd") {
                ...
                iss >> current_material->albedo_factor.r
                        >> current_material->albedo_factor.g
                ...
                iss >> current_material->emissive_factor.r
                        >> current_material->emissive_factor.g
...
auto load_obj(RenderContext &ctx, GlobalCommandContext &cmd_ctx,
        ...
        if (line.starts_with("mtllib ")) {
            ...
            for (std::size_t i = 1; i + 1 < face_vertices.size(); ++i) {
                ...
                for (std::size_t j = 0; j < 3; ++j) {
                    ...
                    glm::vec3 n = (vert_idx[2] > 0)
                                      ? normals[static_cast<size_t>(vert_idx[2] - 1)]
                                      : glm::vec3(0.0f, 1.0f, 0.0f);

                    glm::vec2 uv = (vert_idx[1] > 0)
                                       ? texcoords[static_cast<size_t>(vert_idx[1] - 1)]
                                       : glm::vec2(0.0f);
    ...
    do { \
        if (!(mat).field_name.empty() && !unique_texture_names.contains((mat).field_name)) { \
            load_futures.emplace_back(std::async(std::launch::async, \
                [base_path, tex_name = (mat).field_name]() -> TextureLoadPacket { \
                    return load_texture_from_file(base_path / tex_name, \
                        TextureLoadPacket::Type::t, TextureLoadPacket::Class::clazz); \
                })); \
...
#undef LOAD_MAP
    ...
    for (const auto& tex : textures) {
        auto img = create_image_from_span_v2(
            ...
            tex.height,
            tex.to_format(),
            std::span<const uint8_t>{tex.rgba.data(), tex.rgba.size()},
            tex.name
        );
    ...
    auto material_ids_buffer = Buffer::from_slice<u32>(
        ctx.allocator,
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
    ...
    auto position_vb = mesh.vertices | std::views::transform([](const auto& v) { return v.position; }) | std::ranges::to<std::vector<glm::vec3>>();

    auto vertex_buffer = Buffer::from_slice<Vertex>(ctx.allocator, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, std::span(vb_copy), std::format("vertex_buffer_{}", obj_path.filename().string())).value();
    auto position_vertex_buffer = Buffer::from_slice<glm::vec3>(ctx.allocator, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, std::span(position_vb), std::format("position_buffer_{}", obj_path.filename().string())).value();
    auto index_buffer = Buffer::from_slice<u32>(ctx.allocator, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, std::span(ib_copy), std::format("index_buffer_{}", obj_path.filename().string())).value();
