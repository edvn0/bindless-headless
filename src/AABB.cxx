...
auto create_aabb_device_buffer(
    ...
    return Buffer::from_slice<PackedAABB>(
        allocator,
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        std::span(packed_aabbs),
        name
    );
}

auto update_aabb_device_buffer(
    VmaAllocator& allocator,
    Buffer& buffer,
    std::span<const AABB> aabbs
) -> void {
...
auto create_mesh_aabb_data(
    ...
    auto submesh_aabbs = compute_submesh_aabbs(
        std::span(mesh.vertices),
        std::span(mesh.indices),
        std::span(mesh.submeshes)
    );

    AABB mesh_aabb = merge_aabbs(std::span(submesh_aabbs));

    // Create device buffer
    auto buffer_result = create_aabb_device_buffer(
        allocator,
        std::span(submesh_aabbs),
        std::format("aabb_buffer_{}", name)
    );
