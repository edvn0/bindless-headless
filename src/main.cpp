...
auto execute(int argc, char **argv) -> int {
    ...
    std::array<const std::string_view, 5> names = {
        "LightFlagsCS", "LightScanLocalCS", "LightScanGroupsCS",
    ...
    auto tonemap_pipeline =
            create_tonemap_pipeline(device, *pipeline_cache, bindless.layout, tonemap_code.at(0), tonemap_code.at(1),
                                    "vs_main", "fs_main", VK_FORMAT_R8G8B8A8_UNORM);
    ...
    auto cubes = std::vector<PointLight>(opts.light_count / 10);
    ...
    ResizeGraph resize_graph{}; {
        ...
        const auto tonemapped_node = resize_graph.add_node("tonemapped_image", [&](VkExtent2D e, const ResizeContext & resize_context) {
            const auto old_tonemap = tonemapped_target_handle;


tonemapped_target_handle = ctx.create_texture(
    create_offscreen_target(allocator, e.width, e.height, VK_FORMAT_R8G8B8A8_UNORM,
                            "tonemapped"));
destroy(ctx, old_tonemap,resize_context.retire_value);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        auto predepth_val = submit_stage(
            tl_graphics, device,
            [&](VkCommandBuffer cmd) {
                TracyVkZone(tracy_graphics.ctx, cmd, "Predepth");
