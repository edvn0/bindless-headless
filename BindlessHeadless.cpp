...
auto main(int, char**) -> int
{
	...
	auto simple_random_colour_pipeline = compiler.compile_compute_from_file("shaders/simple_random_colour.slang", "main");
	...
	auto&& [compute_pipeline, compute_layout] = create_compute_pipeline(device,
		pipeline_cache,
		bindless.layout,
	...
	auto handle = ctx.create_texture(create_offscreen_target(
		...
		1280u,
		720u,
		VK_FORMAT_R8G8B8A8_UNORM, "offscreen"));

	auto sampler = ctx.create_sampler(create_sampler(allocator, VkSamplerCreateInfo{
	...
	for (i = 0; i < 10'000; ++i) {
		...
		auto light_val = submit_stage(
			...
			device,
			[&](VkCommandBuffer cmd) {
				// vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, light_cull_pipeline);
				// vkCmdBindDescriptorSets(..., bindless.set, ...);
				// vkCmdDispatch(cmd, ...);
				auto image = ctx.textures.get(handle)->image;
	...
	ctx.textures.for_each_live([&](auto handle, auto&) {
		destroy(ctx, handle);
		});

	ctx.samplers.for_each_live([&](auto handle, auto&) {
		destroy(ctx, handle);
		});
	...
	std::cout << "Bindless headless setup and teardown completed successfully." << std::endl;
	return 0;
}
...
namespace destruction {
	auto bindless_set(VkDevice device, BindlessSet& bs) -> void {
		if (bs.pool)   vkDestroyDescriptorPool(device, bs.pool, nullptr);
		if (bs.layout) vkDestroyDescriptorSetLayout(device, bs.layout, nullptr);
...
namespace detail {
	...
	auto set_debug_name_impl(VmaAllocator& alloc, VkObjectType object_type, std::uint64_t object_handle, std::string_view name) -> void
	{
