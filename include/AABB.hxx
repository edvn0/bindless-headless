...
struct AABB {
    ...
    constexpr AABB(glm::vec3 const& min_point, glm::vec3 const& max_point)
        : min(min_point)
        , max(max_point) {}

    [[nodiscard]] constexpr  auto center() const -> glm::vec3 {
        return (min + max) * 0.5f;
    }

     [[nodiscard]] constexpr auto extent() const -> glm::vec3 {
        return max - min;
    }

     [[nodiscard]] constexpr auto half_extent() const -> glm::vec3 {
        return (max - min) * 0.5f;
    }
    ...
    [[nodiscard]] constexpr auto scaled(float scale) const -> AABB {
...
struct PackedAABB {
    ...
    explicit PackedAABB(AABB const& aabb)
        : min(aabb.min)
        , _pad0(0)
        , max(aabb.max)
        , _pad1(0) {}

    [[nodiscard]] constexpr auto to_aabb() const -> AABB {
        return AABB{min, max};
    }
};
